...
class Product_model extends CI_Model
{
    ...
    public function getProductById($id){
        ...
        $data = $this->db->where('id',$id)->get($this->_db_name)->row();
        return $data;
    }

    public function get_more_product($category,$id,$limit=3){
        $pro = $this->db->where('catagory_id',$category)->where_not_in('id',$id)->limit($limit)->order_by('id','DESC')->get($this->_db_name)->result();
        return $pro;
    }
